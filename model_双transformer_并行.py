...
class DACE(nn.Module):
    ...
    def forward(
        ...
        code_attn_mask = (
                ...
                .unsqueeze(0)
                .expand(B, T) >= code_len.unsqueeze(1)
        )

        # ===== store original states (CRITICAL) =====
        q_in = d_output
        c_in = code_h

        # code attends question (in parallel)
        ...
        # question attends code (in parallel)
        ...
        # ===== independent residual + norm =====
        code_h = self.cross_norm(c_in + code_ctx)
        d_output = self.cross_norm(q_in + q_ctx)
        # ===== MoE fusion =====
