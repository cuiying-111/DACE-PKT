import sys
import os
import os.path
import glob
import logging
import argparse
import numpy as np
import torch
from run import train, test, train_clean
from load_data import inject_noise
import pickle
from config import Config
from load_data import PID_DATA
from utils import setseed, try_makedirs, load_model, get_file_name_identifier
from layers import DisenEstimator

def train_fb_step(model_fb, optim_fb, train_q_data, train_qa_data, train_pid, train_code_data, train_seq_len, valid_q_data, valid_qa_data, valid_pid, valid_code_data,valid_seq_len,  epoch):
    # ================== SAFETY CHECK ==================
    assert train_seq_len.ndim == 1, \
        f"train_seq_len must be (B,), got {train_seq_len.shape}"
    assert valid_seq_len.ndim == 1, \
        f"valid_seq_len must be (B,), got {valid_seq_len.shape}"
    # ==================================================
    model_fb.train()
    train_loss, train_accuracy, train_auc = train(
        model_fb,
        params,
        optim_fb,
        train_q_data,
        train_qa_data,
        train_pid,
        train_code_data,
        train_seq_len,
        label='Train'
    )

    valid_loss, valid_accuracy, valid_auc = test(
        model_fb,
        params,
        valid_q_data,
        valid_qa_data,
        valid_pid,
        valid_code_data,
        valid_seq_len,
        label='Valid'
    )
    print("epoch ", epoch + 1)
    print("valid_auc\t", valid_auc, "\ttrain_auc\t", train_auc)
    print("valid_accuracy\t", valid_accuracy,
            "\ttrain_accuracy\t", train_accuracy)
    print("valid_loss\t", valid_loss, "\ttrain_loss\t", train_loss)


def custom_train(params,train_q_data, train_qa_data, train_pid, train_code_data, train_seq_len, valid_q_data, valid_qa_data, valid_pid, valid_code_data, valid_seq_len):
    
    # ================================== model initialization ==================================
    # ================== SAFETY CHECK (必须) ==================
    assert train_seq_len.ndim == 1, \
        f"train_seq_len must be (B,), but got {train_seq_len.shape}"
    assert valid_seq_len.ndim == 1, \
        f"valid_seq_len must be (B,), but got {valid_seq_len.shape}"
    # =========================================================
    model_fb = load_model(params)
    model_fc = load_model(params)
    optim_fb = torch.optim.Adam(
        model_fb.parameters(), lr=params.lr, betas=(0.9, 0.999), eps=1e-8)
    optim_fc =  torch.optim.Adam(
        model_fc.parameters(), lr=params.lr, betas=(0.9, 0.999), eps=1e-8)
    
    disc = DisenEstimator(params.d_model, params.d_model, dropout=0.2).to(next(model_fc.parameters()).device)
    optim_disc = torch.optim.Adam(disc.parameters(), lr=0.2)
    # dis_scheduler = StepLR(dis_optim, step_size=20, gamma=0.1)
    
    ####### TODO data  这里是第一阶段，fb的训练
    print("#"*20, "f_b training...", "#"*20)
    for epoch in range(0, params.fb_epoch):
        train_fb_step(model_fb, optim_fb, train_q_data, train_qa_data, train_pid, train_code_data, train_seq_len,
                      valid_q_data, valid_qa_data, valid_pid, valid_code_data, valid_seq_len, epoch)
        # test_one_dataset()
    ###### OK
    ############# TODO
    # torch.save({'epoch': params.fb_epoch,
    #                     'model_state_dict': model_fb.state_dict(),
    #                     'optimizer_state_dict': optim_fb.state_dict(),
    #                     },
    #                    f'{params.dataset}_bias_guess_' + str(params.fb_epoch)
    #                    )
    ##################
    
    # 这是第二阶段fc的训练
    print("#"*20, "f_c training...", "#"*20)
    
    all_train_loss = {}
    all_train_accuracy = {}
    all_train_auc = {}
    all_valid_loss = {}
    all_valid_accuracy = {}
    all_valid_auc = {}
    best_valid_auc = 0
   # 核心的训练函数是train_clean(run.py)
    for idx in range(0, params.max_iter):
        train_loss, train_accuracy, train_auc = train_clean(
    params,
    model_fb,
    model_fc,
    optim_fc,
    disc,
    optim_disc,
    train_q_data,
    train_qa_data,
    train_pid,
    train_code_data,
            train_seq_len,
    "Train",
    idx
)
        valid_loss, valid_accuracy, valid_auc = test(
            model_fc,
            params,
            valid_q_data,
            valid_qa_data,
            valid_pid,
            valid_code_data,
            valid_seq_len,
            label='Valid'
        )
        print("epoch ", idx + 1)
        print("valid_auc\t", valid_auc, "\ttrain_auc\t", train_auc)
        print("valid_accuracy\t", valid_accuracy,
                "\ttrain_accuracy\t", train_accuracy)
        print("valid_loss\t", valid_loss, "\ttrain_loss\t", train_loss)

        try_makedirs('model')
        try_makedirs(os.path.join('model', params.model))
        try_makedirs(os.path.join('model', params.model, params.save))

        all_valid_auc[idx + 1] = valid_auc
        all_train_auc[idx + 1] = train_auc
        all_valid_loss[idx + 1] = valid_loss
        all_train_loss[idx + 1] = train_loss
        all_valid_accuracy[idx + 1] = valid_accuracy
        all_train_accuracy[idx + 1] = train_accuracy

        # output the epoch with the best validation auc
        if valid_auc > best_valid_auc:
            path = os.path.join('model', params.model,
                                params.save,  file_name) + '_*'
            for i in glob.glob(path):
                os.remove(i)
            best_valid_auc = valid_auc
            best_epoch = idx+1
            torch.save({'epoch': idx,
                        'model_state_dict': model_fc.state_dict(),
                        'optimizer_state_dict': optim_fc.state_dict(),
                        'loss': train_loss,
                        },
                       os.path.join('model', params.model, params.save,
                                    file_name)+'_' + str(idx+1)
                       )
        if idx-best_epoch > 10:  # 这里面是早停机制，如果连续 10 个 epoch 验证 AUC 没有提升，则提前停止
            break  
    try_makedirs('result')
    try_makedirs(os.path.join('result', params.model))
    try_makedirs(os.path.join('result', params.model, params.save))
    f_save_log = open(os.path.join(
        'result', params.model, params.save, file_name), 'w')
    f_save_log.write("valid_auc:\n" + str(all_valid_auc) + "\n\n")
    f_save_log.write("train_auc:\n" + str(all_train_auc) + "\n\n")
    f_save_log.write("valid_loss:\n" + str(all_valid_loss) + "\n\n")
    f_save_log.write("train_loss:\n" + str(all_train_loss) + "\n\n")
    f_save_log.write("valid_accuracy:\n" + str(all_valid_accuracy) + "\n\n")
    f_save_log.write("train_accuracy:\n" + str(all_train_accuracy) + "\n\n")
    f_save_log.close()
    
    return best_epoch






def test_one_dataset(params, file_name,
                     test_q_data, test_qa_data, test_pid, test_code_data, test_code_len_data,
                     best_epoch):

    print("\n\nStart testing ......................\n Best epoch:", best_epoch)

    # ============================================================
    # 1. 构建模型（fc 模型）
    # ============================================================
    model = load_model(params)

    # ============================================================
    # 2. 加载 checkpoint
    # ============================================================
    checkpoint_path = os.path.join(
        'model', params.model, params.save, file_name
    ) + '_' + str(best_epoch)

    checkpoint = torch.load(
        checkpoint_path,
        map_location=next(model.parameters()).device,
        weights_only=False
    )

    # ============================================================
    # 3. 加载模型参数
    # ============================================================
    model.load_state_dict(checkpoint['model_state_dict'])
    model.has_warmup = True
    model.eval()

    # ============================================================
    # 4. 测试（双序列）
    # ============================================================
    test_loss, test_accuracy, test_auc = test(
        model,
        params,
        test_q_data,
        test_qa_data,
        test_pid,
        test_code_data,
        test_code_len_data,
        label='Test'
    )

    # ============================================================
    # 5. 输出结果
    # ============================================================
    print("\ntest_auc\t", test_auc)
    print("test_accuracy\t", test_accuracy)
    print("test_loss\t", test_loss)
    
if __name__ == '__main__':

    params = Config

    print("\n", "#" * 20, 'parameters', "#" * 20)
    for key in params.__dict__:
        if not key.startswith("__"):
            print('\t', key, '\t', params.__dict__[key])

    file_name = params.file_name

    # --------------------------------------------------
    # Load data
    # --------------------------------------------------
    setseed(params.seed)

    dat = PID_DATA(
        n_question=params.n_question,
        seqlen=params.seqlen,
        dataset_name=params.dataset,
        device=params.device
    )

    train_data_path = params.data_dir + "/train.jsonl"
    valid_data_path = params.data_dir + "/valid.jsonl"
    test_data_path  = params.data_dir + "/test.jsonl"



    # train
    train_q_data, train_qa_data, train_pid, train_code_data,  train_code_mask, train_seq_len = dat.load_data(
        train_data_path, cache_name="train"
    )

    # inject bias (optional, DACE only)
    if params.bias_type != "None":
        interactions_per_student = np.sum(train_q_data > 0, axis=1)

        # 从 QA 中恢复 answer 序列
        ans_seq = (train_qa_data - train_q_data) / params.n_question

        ans_seq = inject_noise(
            ans_seq,
            interactions_per_student,
            params.inject_p,
            params.bias_p,
            params.bias_type,
            params.dataset
        )

        # 重新组合 QA
        train_qa_data = train_q_data + ans_seq * params.n_question

    # valid / test（不注入噪声）
    valid_q_data, valid_qa_data, valid_pid, valid_code_data, valid_code_mask, valid_seq_len = dat.load_data(
        valid_data_path, cache_name="valid"
    )

    test_q_data, test_qa_data, test_pid, test_code_data,  test_code_mask, test_seq_len = dat.load_data(
        test_data_path, cache_name="test"
    )


    print("\n", "#" * 20, 'data', "#" * 20)
    print("train_q_data.shape :", train_q_data.shape)
    print("train_qa_data.shape:", train_qa_data.shape)
    print("valid_q_data.shape :", valid_q_data.shape)
    print("valid_qa_data.shape:", valid_qa_data.shape)
    print("test_q_data.shape  :", test_q_data.shape)
    print("test_qa_data.shape :", test_qa_data.shape)

    # =====================================================
    # <<< 验证 code 序列是否与 KT 序列对齐（必须在这里）
    # =====================================================
    print("train_code_data.shape :", train_code_data.shape)  # (B, T, D)
    print("train_code_mask.shape:", train_code_mask.shape)  # (B, T)
    print("train_seq_len.shape  :", train_seq_len.shape)  # (B,)
    print("train_seq_len[:10]   :", train_seq_len[:10])
    # =====================================================

    # --------------------------------------------------
    # Train & Test
    # --------------------------------------------------
    
    best_epoch = custom_train(
        params,
        train_q_data, train_qa_data, train_pid, train_code_data,train_seq_len,
        valid_q_data, valid_qa_data, valid_pid, valid_code_data,valid_seq_len
    )

    test_one_dataset(
        params,
        file_name,
        test_q_data,
        test_qa_data,
        test_pid,
        test_code_data,
        test_seq_len,
        best_epoch
    )
