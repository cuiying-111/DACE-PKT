...
from codeEncoder import CodeSequenceGRUEncoder
...
class DACE(nn.Module):
    def __init__(
        ...
        # ===== Added for code sequence =====
        self.code_encoder = CodeSequenceGRUEncoder(
            code_emb_dim=code_emb_dim,
            hidden_dim=code_hidden_dim,
        )
        # ===== 门控融合修改 [ADD] 把 code 投影到 d_model =====
        self.code_proj = nn.Linear(code_hidden_dim, d_model)

        # ===== 门控：只看 (d_output , code_proj) =====
        self.code_gate = nn.Linear(d_model * 2, 1)
    ...
    def forward(
        ...
        if code_emb is not None:
            ...
            code_h = self.code_proj(code_h)  # ←【关键新增】  # 门控融合修改
        else:
            B, T, _ = d_output.size()
            code_h = torch.zeros(B, T, d_output.size(-1), device=device)

        # ===== Fusion =====
        # concat_p = torch.cat([d_output, p_embed_data, code_h], dim=-1)
        # output_hidden = self.out(concat_p)
        # ===== 门控融合修改 Gated fusion (REPLACE concat) =====
        gate_input = torch.cat([d_output, code_h], dim=-1)  # (B, T, d_model + code_hidden_dim)
        gate = torch.sigmoid(self.code_gate(gate_input))  # (B, T, 1)

        fused_state = d_output + self.code_alpha * gate * code_h

        concat_p = torch.cat([fused_state, p_embed_data], dim=-1)
        output_hidden = self.out(concat_p)
        # ===============================================
