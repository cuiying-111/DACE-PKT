...
PROJECT_CODENET_DATA = (
    "/home/cuiying/projects_paper/DACE-main/data/codenet/Project_CodeNet/code_text"
)
class PID_DATA(object):
    """
    原始 DACE PID_DATA
    仅修改：
    - 从 code_path_seq 读取代码文本（AIZU / AtCoder）
    其余逻辑一律保持不变
    """
    ...
    def load_data(self, jsonl_path, cache_name="train"):
        ...
        with open(jsonl_path, "r", encoding="utf-8") as f:
            for line in f:
                ...
                # 唯一修改点：读取代码文本
                # ------------------------------
                if "code_seq" in record:
                    # 原始 DACE 数据集
                    code_seq = record["code_seq"]

                elif "code_path_seq" in record:
                    # AIZU / AtCoder
                    code_seq = []
                    for rel_path in record["code_path_seq"]:
                        ...
                        if os.path.isfile(abs_path):
                            with open(abs_path, "r", encoding="utf-8", errors="ignore") as fin:
                                code_seq.append(fin.read())
                        else:
                            # 保序列长度一致
                            code_seq.append("")

                else:
                    raise ValueError("No code_seq or code_path_seq found in record")

                assert len(pid_seq) == len(qid_seq) == len(a_seq) == len(code_seq)

                user_records.append((pid_seq, qid_seq, a_seq, code_seq))
