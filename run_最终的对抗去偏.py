...
def train_clean(params,
    ...
    if params.disentangle:
        for idx in range(N):
            ...
            loss_c, _, _, fused_c = model_fc(
                input_q, input_qa, target, input_pid,
                code_emb=code_emb, code_len=code_len,
                return_output=True,
                return_fused=True  # 新增参数
            )

            with torch.no_grad():
                loss_b, _, _, fused_b = model_fb(
                    input_q, input_qa, target, input_pid,
                    code_emb=code_emb, code_len=code_len,
                    return_output=True,
                    return_fused=True  # 新增参数
            ...
            fused_hidden_b = get_sequence_z(fused_b, target > -0.9).detach()
            fused_hidden_c = get_sequence_z(fused_c, target > -0.9).detach()

            dis_loss = -disc(fused_hidden_b, fused_hidden_c)
    ...
    for idx in range(N):
        ...
        loss_c, pred, true_ct, fused_c = model_fc(
            input_q, input_qa, target, input_pid,
            code_emb=code_emb, code_len=code_len,
            return_output=True,
            return_fused=True  # 新增参数
        ...
        fused_hidden_b = get_sequence_z(fused_b, target > -0.9).detach()
        fused_hidden_c = get_sequence_z(fused_c, target > -0.9)
