...
# ===== MoE fusion =====
class DynamicMoEFusion(nn.Module):
    def __init__(
        self,
        input_dim,
        output_dim,
    ...
    def forward(self, x):
        """
        x: (B, T, D) where D = d_output ⊕ code_h
        """
        B, T, D = x.size()

        # -------- Global context (AMoFE-style) --------
        gap = x.mean(dim=1)          # (B, D)
        gmp, _ = x.max(dim=1)        # (B, D)
        z_gate = gap + gmp           # (B, D)

        gate_logits = self.gate_fc(z_gate)  # (B, E)

        if self.noisy_gating:
            noise = self.noise_fc(z_gate)
        ...
        for expert in self.experts:
            out = expert(x)  # (B, T, output_dim)
            expert_outputs.append(out)

        expert_stack = torch.stack(expert_outputs, dim=-1)  # (B, T, output_dim, E)

        fused = torch.sum(
            expert_stack * gate_weights.unsqueeze(1).unsqueeze(2),
            dim=-1
        )  # (B, T, output_dim)
...
class DACE(nn.Module):
    def __init__(
        ...
        # ===== MoE fusion =====
        self.moe_fusion = DynamicMoEFusion(
            input_dim=d_model * 2,
            output_dim=d_model,
            num_experts=num_experts,  # ← 超参数
            noisy_gating=True  # ← 强烈建议
        )
    ...
    def forward(
        ...
        moe_out, moe_gate = self.moe_fusion(moe_input)
        ...
        if not return_output:  # 总损失 = 主损失 + 0.1 * 对比学习损失
            ...
            if return_fused:  # 新增：返回MOE融合特征
                return (
                    loss.sum() + 0.1 * cl_loss,
                    torch.sigmoid(preds),
                    mask.sum(),
                    fused_state,  # 返回MOE融合特征而不是d_output
                )
            else:  # 保持原样，返回题目分支特征
                return (
                    loss.sum() + 0.1 * cl_loss,
